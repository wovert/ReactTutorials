...
> Store对象包含所有数据，想得到某个时点的数据，就要对 Store 生成快照。这种**时点的数据集合**，就叫做 State
...
上面代码中，第二个`dispatch`方法发出的是异步 `Action`，只有等到操作结束，这个 `Action` 才会实际发出。注意，createAction的第二个参数必须是一个 Promise 对象。
