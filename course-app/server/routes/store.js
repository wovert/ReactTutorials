...
route.post("/add", (req, res) => {
  ...
  req.session.storeList = !req.session.storeList ? [] : req.session.storeList;
...
route.post("/remove", (req, res) => {
  ...
  if (personID) {
    req.storeDATA = req.storeDATA.filter(item => {
      return !(
        parseFloat(item.courseID) === courseID &&
...
route.get("/info", (req, res) => {
  let state = parseFloat(req.query.state) || 0,
    ...
    storeList = [];
  if (personID) {
    // 登录状态下是从 JSON 文件中获取：在 STORE.JSON 中找到所有 personID 和登录用户相同的(服务器从 SESSION 中可以获取用户 ID 的)
